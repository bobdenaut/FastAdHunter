...
impl Server {
        ...
        F: Fn() -> Proxy + Send + Sync + 'static,
    {
        ...
        let rotation = Rotation::new(senders);
...
pub(crate) enum Dispatch {
    Shared(Arc<Proxy>),
    SharedTls(Arc<TlsProxy>),
    Domains { rotation: Rotation, lane: Lane },
}
    ...
    async fn dispatch(&mut self, accepted: Accepted<TcpStream>) {
        match self {
            Self::Shared(proxy) => {
                ...
                tokio::spawn(
                    accepted.serve(move |stream, peer| proxy.serve_connection(stream, peer)),
                );
            }
            Self::SharedTls(proxy) => {
                let proxy = Arc::clone(proxy);
                tokio::spawn(
                    accepted.serve(move |stream, peer| proxy.serve_connection(stream, peer)),
                );
            }
            Self::Domains { rotation, lane } => {
                let Some(accepted) = accepted.detach() else {
                    return;
                };
                rotation.send(lane.handoff(accepted)).await;
            }
        }
    }
}
...
impl Lane {
    fn handoff(&self, accepted: Accepted<std::net::TcpStream>) -> Handoff {
        match self {
            Self::Http => Handoff::Http(accepted),
            Self::Https(proxy) => Handoff::Https(accepted, Arc::clone(proxy)),
        }
    }
}
...
impl Rotation {
    fn new(senders: Vec<mpsc::Sender<Handoff>>) -> Self {
        Self { senders, next: 0 }
    }
    ...
    async fn send(&mut self, mut handoff: Handoff) {
