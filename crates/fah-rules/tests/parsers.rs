...
use fah_rules::{parse_rule_list, InactiveReason, MatcherBuilder, RuleFormat, RuleKind};
...
fn path_qualified_rules_never_block_the_whole_domain() {
    ...
    for (rule, domain) in RULES {
        ...
        // It is not dropped — it is exactly what the HTTP phase activates.
        ...
        // positional check.
        let inactive: Vec<_> = parsed
            .rules
            .iter()
            .filter(|parsed| matches!(parsed.kind, RuleKind::Inactive(_)))
            .collect();
        assert_eq!(
            inactive.len(),
            1,
            "{rule} should be the only inactive rule in the fixture"
        );
        assert_eq!(
            inactive[0].kind,
            RuleKind::Inactive(InactiveReason::UrlPattern),
            "{rule} belongs in the URL-pattern bucket for p2-03"
        );
    }
}
...
fn real_easylist_head_is_detected_as_adblock() {
    ...
    assert_eq!(result.inactive_count(), 30);
}
