...
fn main() -> ExitCode {
    ...
    if args.healthcheck {
        // Phase 0 healthcheck is process-level: config loads, exit 0/1
        // (ARCHITECTURE.md §Docker: distroless has no shell, so the
        // container `HEALTHCHECK` self-execs this binary).
