...
| | |
| --- | --- |
| Branch | **Phase 3 landed on `main`** on 2026-09-13, by fast-forward — `bc49e4e..78238b4`, 105 commits, no merge commit ([plan/plan-merge.md](../plan/plan-merge.md), all five steps closed). Twenty-one commits of p3-10, p3-10b, p3-10c and p3-11 followed. `main` is now `9aacdd4` and is **one commit ahead of both remotes; that commit is not pushed** — `origin/main` and `backup/main` are both at `180beb8`. `phase3-06` (`78238b4`) has served its purpose and sits well behind. Rollback tags: `main-pre-phase3-merge` = `ebc46f1`, `phase3-06-pre-main-merge` = `185139b`; `eb693e2` is the merge commit inside the branch, two parents. `pre-alloc-domain-2026-09-06` = `64be513` stays the rollback point before the allocation domains |
| Tree | **clean — nothing modified, nothing untracked.** `.gitignore` is no longer uncommitted: `93fb85f` took it, ignoring the soak churn by pattern and tracking the collector, and its safety stash was dropped with that commit. One stash remains, `stash@{0}` ("phase3-06 project-state Next row"); it predates this work and is not ours. The last file to land was [allocation-oracles-that-only-hold-on-an-idle-machine.md](solutions/design-patterns/allocation-oracles-that-only-hold-on-an-idle-machine.md), committed `9aacdd4` |
| Tests | **The gate ran on the tip, `9aacdd4`, 2026-09-14, Windows dev box: `cargo fmt --all -- --check` and `cargo clippy --workspace --all-targets -- -D warnings` clean, `cargo test --all-features --workspace --no-fail-fast` 62 targets, 1 607 passed, 0 failed, 12 ignored.** The `e2e` `WSAEACCES` trap (§Known-good gate note) did not fire on this run. The dashboard ran on the same tip and is green: `npm run typecheck` clean, 58 test files, 1 048 tests, 0 failures — which covers the one dashboard change since 2026-09-13, p3-10c's four-line wording edit to `engine-card.tsx` in `180beb8`. Step 4 of the merge ran on the merged tree, 2026-09-13: 58 suites and 0 failures. The targeted checks pass — F1's three TCP tests, F3's allocation ceilings, F10's shutdown flush, H1-H3/D1's `proxy_alloc`, the idle-pool reaper — and F11's wiring was read in `main.rs:769` and `:775-776`. Phase 3's surface passes: `interception` 45/45, `security_phase3` 7/7, `sni` 11/11, `e2e_https` 3/3, `interception_migration` 6/6, and the dashboard's `npm run typecheck` was clean over 1 048 tests. Bench A/B against `main` over four alternating rounds: **no regression demonstrated** |
...
| **Next** | **What can start today, needing no decision: the dev-box half of p3-11.** Part of it has already landed — `crates/fastadhunter/tests/shipped_path_e2e.rs` (`095163b`) walks DNS over UDP and TCP, plain HTTP, an SNI block, an SNI splice through an allocation domain, DoT and DoH in one run with `clients` empty, every layer carrying an allowed control beside the blocked case. What is left there: the security suite, the SNI/DoT/DoH budget rows, Private DNS, the deploy guide's HTTPS section. **Blocked on the deploy decision:** p3-10's B2 sweep on the RB5009 (it also owes p3-11 the dst-nat 443 rule) and p3-11's device half with its seven-day soak. **Their own go, each a deliverable rather than a step:** the DoT/DoH load generator, which does not exist, and a verified runner for splice RSS. In flight and needing nobody: the **0.3.4 soak ends ~2026-09-18**, and its counters settle the F1 `tcp_max_connections` default (§Risk inventory close-out). Interception ships **disabled** through an empty client scope, not through `engine.mode` — and since 2026-09-13 that is the owner's decision rather than a default awaiting a switch: `p3-06` and `p3-06b` are `PARKED`, their non-interception arms moved to p3-11, and the phase's definition of done no longer names interception. Off the critical path: the dashboard's dead `fallback` mode branch (`derive.ts`, `degraded-banner.tsx`), dashboard settings metadata for `runtime.http_runtimes`, the TLS / lol_html capacity microbench. **Before Phase 3 goes live:** the router refuses UDP 443 LAN→WAN, or HTTP/3 bypasses the steer. The rule's **placement was corrected 2026-09-14** — appended behind a final drop it does nothing — and the DNS escape routes are named beside it ([deploy-rb5009.md](deploy-rb5009.md), `73d79aa`) |
...
Still holding, none of them blocking:
