...
**Last updated:** 2026-09-16 (thirteenth pass — a second hot-path audit, this
one covering the listeners the first never entered; four of its findings fixed,
a fifth measured rather than argued; tree clean, both remotes current)

## Now

| | |
| --- | --- |
| Branch | **Phase 3 landed on `main`** on 2026-09-13, by fast-forward — `bc49e4e..78238b4`, 105 commits, no merge commit ([plan/plan-merge.md](../plan/plan-merge.md), all five steps closed). `main` is at **`9cbbfd1`** — Finding 9's HTTPS measurement — and **both remotes carry it**: `git ls-remote` puts `origin/main` and `backup/main` on `9cbbfd1` too, so nothing is unpushed. Read the remotes that way, not from the tracking refs. **Never read the tip from this row** — it has been wrong on seven occasions now (`99e4953`, `9d9d792`, `26ffe1c`, `3e97d16`, `e30af33`, `19693be`, `bb15de7`), so use `git rev-parse main`. Six commits followed `f12eec3`, **three of them production**, and every one comes from the second hot-path audit: **`3ce7ec5`** the poisoned cache shard plus the audit document, **`c93e2d1`** the stale-serve key clone, **`bb15de7`** the rotation error logged once, `f51f70a` this file, `f51a830` Finding 7's extra oracle batches, `9cbbfd1` Finding 9's HTTPS cases with the audit update — §Second hot-path audit below. Before them, fourteen commits followed `9d9d792` and exactly two were production: `e8e7cf8` the enumeration sweep's record, `2b03a30` `bd6b1f0` `0662df4` `8abb9c1` the first hot-path audit and its rewrites, **`26ffe1c` the A1/A2 listener fix**, `ad12103` this file, `3e97d16` the hook and its test, `59a4f84` `847d369` this file again, `76427d9` that audit's close with hard rule 3, `e30af33` `CLAUDE.md`, `514d935` `f12eec3` this file, and **`19693be` the errno probe with A2's classification narrowed** — §Hot-path audit and the A1/A2 fix below. Everything before them is in `git log`; the ten from 2026-09-14/15 are listed in §Session 2026-09-14/15. `phase3-06` (`78238b4`) has served its purpose and sits well behind. Rollback tags: `main-pre-phase3-merge` = `ebc46f1`, `phase3-06-pre-main-merge` = `185139b`; `eb693e2` is the merge commit inside the branch, two parents. `pre-alloc-domain-2026-09-06` = `64be513` stays the rollback point before the allocation domains |
...
| Tests | **The gate ran at `9cbbfd1`, 2026-09-16, Windows dev box:** `cargo fmt --all -- --check` and `cargo clippy --workspace --all-targets -- -D warnings` clean, `cargo test --all-features --workspace` **1 652 passed / 0 failed** (1 651 at `19693be`, 1 648 at `26ffe1c`, 1 635 at `9d9d792`). **The whole audit added exactly one test**, and that asymmetry is the point rather than an oversight: `fah-dns --lib` is at 224 (223 before) for `a_shard_poisoned_by_a_panicking_sweep_still_serves_and_stores`, which was falsified — restoring `.unwrap()` fails it on `PoisonError` — while the key-clone removal is enforced by the borrow checker and the rotation fix is a log level, which nothing in this workspace can assert because no test captures tracing output. `fah-http --lib` therefore stays at 92. The two oracle commits move no count either: `f51a830` raises `intercept_alloc`'s batches from 4 to 6, and `9cbbfd1` adds three `RecordType::HTTPS` cases **inside** existing test functions — both widen evidence rather than add tests. The HTTPS cases are the ones that matter: every DNS allocation figure before them was measured on `RecordType::A`, the one type that builds no `QueryType::Other` string, so all of them were a lower bound. Measured at 64 handles: cache hit 704 observed against a 708 ceiling (11 per query against the A case's 10), miss 1 093 against 1 156 (~17.1 against ~16.3) — **+1 allocation per query** — while blocked HTTPS is 768 against 772, 12 against 13, one *fewer*, because a non-A/AAAA block synthesizes no record. Earlier and still current: `fah-common --lib` 58 (44 before `26ffe1c` — `retry` carried its four moved tests plus two for `never_fatal`, `throttle` brought nine). The dashboard figures below were measured at `9d9d792` and were not re-run for a change that touches no frontend code. From the earlier passes and still current: the listener fix added twelve tests, R1 one and F14 two, so `fah-config --lib` is at 98 (97 after the listener fix, 90 before it) and `fah-api --test api` 138 (133). Both of the last two fixes are mutation-verified: for R1, deleting the two new arms fails its test on `UnknownEnvKey` and pointing one arm at the wrong field fails it on the value; for F14, reverting the atomic fails the behavioural test while deleting the `post_config` call fails the wiring test and leaves the behavioural one passing — which is the proof the two cover different halves. `shipped_path_e2e` now carries six tests in 21 s — four added on 2026-09-15 for the HTTPS listener's own edge cases, the last of them mutation-verified (`max_connections: 2` fails its negative control). The dashboard ran on the same tip and is green: `npm run typecheck` clean, **58 test files, 1 043 tests, 0 failures** (three fewer than the 1 046 of 2026-09-14 — the `fallback` mode's tests went with the mode in `5b5d3e8`), and `npm run build` is under budget at 136 921 B gzip against 153 600 B. `cargo bench -p fastadhunter --bench pipeline --no-run` builds for the first time since p5-04, with no profile override (`3a1b5b8`). The `e2e` `WSAEACCES` trap (§Known-good gate note) did not fire. Bench A/B against `main` over four alternating rounds: **no regression demonstrated** |
...
Fixed and pushed: **`3ce7ec5`** Finding 1 — a poisoned shard answered no query
on it ever again, silently, and `record_task_death` could not see it because
the sweep continues rather than ends; **`c93e2d1`** Finding 4; **`bb15de7`**
Finding 5; **`f51a830`** Finding 7.

Open: **2** (HIGH) and **3** need the RB5009. Finding 2's severity comes from
`http_runtimes = 2` — it costs one of the two threads carrying all proxy
traffic, and DNS is unaffected, which is why it defers rather than blocks.
**9** is measured rather than argued since `9cbbfd1`, so what is left of it is a
decision on its step 2, not work. **6**, **8** and **10** carry no date, and
**8 is not the test-only item it looks like** — `udp` and `tcp` are private
modules exporting neither `run` nor their listener traits, so the oracle needs a
production visibility change before it can be written.
