...
**Last updated:** 2026-09-19 (twentieth pass — **the workspace went to 0.4.1**:
one line in `Cargo.toml` and the lockfile refresh that follows it, uncommitted
and untagged, with nothing else touched. **No doc followed it** —
[0.4.0-install.md](0.4.0-install.md) still names 0.4.0 everywhere and still tells
the operator to check out a `v0.4.0` tag that does not exist, so the guide and a
build from the tip now disagree on the number (§Version). The nineteenth pass,
...
| **Next** | ~~Smallest thing available today, test-only and needing nobody: the second audit's >64 KiB TCP reply test~~ — **done 2026-09-17 in `067427a`**, and it did not pin what it was expected to: hickory's encoder refuses to grow past 65 535 and sets `TC` itself, so `encode_for_transport`'s budget comparison was never the last guard; the test pins the size, the bit and that the two-byte prefix equals the payload. **The one open item on the second audit that is not the test-only work it looks like:** Finding 8's listener-layer oracle, because `lib.rs` declares `mod udp` and `mod tcp` private and exports neither `run` nor `Datagrams`/`Accept`, so an oracle in `tests/` needs a production visibility change or a feature gate first. Finding 9's step 2 is **done** — the `QueryType` redesign shipped in `af5cf61`, validated on the RB5009 — so it is no longer a pending decision. **The larger dev-box item is still the dev-box half of p3-11.** Part of it has landed — `crates/fastadhunter/tests/shipped_path_e2e.rs` (`095163b`) walks DNS over UDP and TCP, plain HTTP, an SNI block, an SNI splice through an allocation domain, DoT and DoH in one run with `clients` empty, every layer carrying an allowed control beside the blocked case; the security suite landed 2026-09-14 (`00698ee`); on 2026-09-15 four more binary-level arms closed two of the coverage table's three `no` rows (`d93f4ad`, `5f91db5`), leaving HTTPS lane saturation, which was deliberately left open. The deploy guide's HTTPS section is done too: §5c was audited against the live router on 2026-09-15 and three defects fixed (`dac5be0`) — rollback left the two QUIC rejects behind in `filter`, the intro counted two rules where there are up to seven, and the order was implicit, though the QUIC reject has to land before the dst-nat or a browser on HTTP/3 walks past the steer. **What is left on the dev box is Private DNS on the device, which waits for the deploy anyway. The SNI/DoT/DoH budget rows are not dev-box work** — every figure in PERFORMANCE.md's table is a column headed `Measured on the RB5009`, and its SNI and DoT/DoH rows say `TBD — set from the RB5009` because loopback and TLS legs do not convert (§Conversion, line 141). A dev-box bench is a diagnostic on the way to those rows, never the row itself. **Blocked on the deploy decision:** p3-10's B2 sweep on the RB5009 (it also owes p3-11 the dst-nat 443 rule) and p3-11's device half with its seven-day soak. **The second audit's Findings 2 and 3 are no longer here** — both were measured on the RB5009 via a diag-timing probe on veth3 (`172.17.0.4`, production untouched) and closed with no code change; §Second hot-path audit records the numbers. **Its TODO row 11, the TCP length-prefix framing, is gone too** — measured on the dev box 2026-09-16 and closed the same way (`52eec09`), which also retires `A3` from the older audit. **Row 9 closed 2026-09-17 (`067427a`)**, so rows 8 and 10 are the audit's whole open surface. **Their own go, and both waiting on more than a decision to write them:** the DoT/DoH load generator and a verified runner for splice RSS. Neither can produce an interpretable figure before p3-11 fixes the budgets on the device — PERFORMANCE.md's `DoT / DoH added latency vs UDP, p50` is still `TBD` and the TLS/HTTP legs do not convert from x86 ([p3-10-track-b1-x86.md](code-review/phase3/p3-10-track-b1-x86.md) §Remaining TODOs, `e028328`). ~~In flight and needing nobody: the 0.3.4 soak ends ~2026-09-18~~ — **it was stopped on day 5, 2026-09-16** (§0.3.4 soak). Its F1 `tcp_max_connections` counters were read before the stop and are usable; **the DoT half was never readable on that build**, which predates the `p3-10b` gauge. **The owner has decided the next build is 0.4.0 and wants full `dns+http+https` — SNI, DoT and DoH working.** The deploy config is now in the repo at [fastadhunter.toml](fastadhunter.toml) (`98eda8d`, raised for 0.4.0 in `be54185`) and validates against the binary's own `--healthcheck`: `mode = "dns+http+https"`, `dot_enabled`/`doh_enabled` true with `dot_port` 853, `[https.listen]` on 8444, and **`[https.sni] no_sni = "block"`** — owner's choice, taken to make no-SNI connections visible in the live feed. `api.address` stays `0.0.0.0` by decision, so **DoH is IPv4-only**. **The version bump is done (`0197ae5`) and the deploy is now written down** — [0.4.0-install.md](0.4.0-install.md) (`2b47737`) carries the exact RouterOS commands for every missing setting, checked against the live router on 2026-09-17, so the build and then the deploy are the next steps and both are the owner's to run. **Phase 3's SNI/DoT/DoH was proven by execution the same day** (§Phase 3 verified), so the deploy decision no longer waits on any doubt about the code. **One new thing sits in front of the build: the workspace went to 0.4.1 on 2026-09-19 and is uncommitted**, so the guide and the tip disagree on the number — either deploy 0.4.1 and raise the guide with it, or keep 0.4.0 and drop the bump. That is the owner's call and nothing else depends on it (§Version). **One dated item runs beside all of this and is not part of it: the Cloudflare token is empty and the public certificate's renewal window opens 2026-11-08** (§Public certificate) — after the deploy that certificate also backs DoT, so an expiry stops Android Private DNS silently rather than merely warning in a browser. Gates were green at `455ea36` and the bump changes only a version string. Interception ships **disabled** through an empty client scope, not through `engine.mode` — and since 2026-09-13 that is the owner's decision rather than a default awaiting a switch: `p3-06` and `p3-06b` are `PARKED`, their non-interception arms moved to p3-11, and the phase's definition of done no longer names interception. Off the critical path: dashboard settings metadata for `runtime.http_runtimes` — a deliberate omission, since `metadata.ts` is an acknowledged subset and the raw All-settings panel renders every unmodelled key — and alloc 11b (`Connection::graceful_shutdown` into `Proxy::serve_connection`; 11a already keeps DNS answering through the drain). **Before Phase 3 goes live:** the router refuses UDP 443 LAN→WAN, or HTTP/3 bypasses the steer. The rule's **placement was corrected 2026-09-14** — appended behind a final drop it does nothing — and the DNS escape routes are named beside it ([deploy-rb5009.md](deploy-rb5009.md), `73d79aa`) |
...
## Deploy guide — 2026-09-17, written from the live router
