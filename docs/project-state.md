...
| | |
| --- | --- |
| Ran | SNI **pass** · P6 **pass** · P7-store **pass** · P4 ran 3× valid, **row-setter withdrawn** (below) · P4-LAN, P8-probe, P9-probe diagnostics · P1-loopback no pick · **P5 FAIL** |
| P4 | **DoT and DoH rows return to `TBD`, 2026-09-05.** Three valid in-device sessions of the same workload on the same build give DoT added +61 / +17 / +16 µs — the declared `p50(transport) − p50(UDP)` statistic subtracts a UDP control that moved 73 % between sessions (101 / 175 / 148 µs) while the difference is ~1/6 of either operand. **Transport paths are healthy and no code change follows**: DoT 162–192 µs and DoH 1 016–1 044 µs absolute both beat the ×9 prediction (≈ 405 / ≈ 1 422 µs), and DoH is the most stable quantity in the set at ~3 %. The fix is the statistic — pool the UDP control across sessions. Client/server CPU attribution is **unresolved**: `/tool/profile` charges all container work to one aggregate task and never named the harness apart from the server |
| P5 | **closed 2026-09-05.** Fails `< 1 ms` at **1.389 ms** under the plan's frozen pass-vs-pass statistic, which is not reinterpreted. The increment **depends on the CPU speed regime the device is in, not on the mint**: reproducibly 1.25–1.39 ms across four sessions, but 0.728 ms paired per host (C2) with 74 of 240 pairs still over 1 ms. `certs_mint` passes independently at **450.88 µs** (D11-on-device, 8.4× the dev box, so the ~9× factor holds) and stays the row-setter. The two questions P5-diag left open are answered in §P5-regime — 944.5 µs is a mixed-regime median against fastest listener rows of 513–522 µs (+14 %), and the +647 µs was the repeat arm's advantage from its own dense pass, not a first-sight cost. Mechanism inferred from 1 : 2 : 4 interval levels including the crypto-free `dispatch_wait` path; no per-connection clock trace exists. **Owner disposition: a recorded budget miss, not a demonstrated defect — does not block Phase 3 closure. No further experiment, no `fah-certs` change** |
...
| `SPLICE_BUF` | **decided 2026-09-05: 16 KiB per direction stays, budget stays 32 MiB, `max_connections` unmoved.** Every candidate clearing 0.9 × best exceeded the 32 MiB budget, and the sweep establishes no CPU-per-relayed-byte advantage (no `/tool/profile` share). No code or config change follows. **Not a P1 result** — P1-LAN still owes the ≥ 100 MiB/s confirmation and is parked; with no pick it now carries one build, not two |
