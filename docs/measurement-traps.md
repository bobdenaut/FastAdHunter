...
| A stable control arm on a non-idle box | **The ±5 % control-arm rule protects A/B verdicts, not tip-only absolutes** (p3-06 session S3, 2026-09-03). A browser plus a fullscreen video on a second monitor ran through a whole 22-arm session: the control arm `http_pass_through/direct_to_origin` moved +0.4 % and every interleaved A/B delta held — while the identical A-side matcher exe read +5.9 % against the previous day, `certs_mint` +6–7 % and the steady-state splice 10–25 % lower with its direct control unchanged. Interleaving cancels background load; a tip-only series has nothing to cancel it against. Before any tip-only figure: `tasklist` for browsers and players, and record that it was checked. |
...
| `https_handshake` legs that go through the proxy, compared across runs | **Bimodal per leg per run on the dev box, and `TIME_WAIT` is not the cause** (p3-07 review A/B, 2026-09-10). Same binary, `TIME_WAIT` drained below 200 sockets before every arm, `direct_to_origin` flat at 1.00–1.05 ms: `spliced` read 1.60 ms on the first arm and 8.7–9.3 ms on the next three, while `https_h2_download/intercepted` did the reverse (86 ms first, 15–17 ms after). A base/tip pair inside one regime compares; a pair straddling regimes reports ±5× on untouched code. Interleave, read the untouched `spliced` leg as the control for the proxy path, and discard any pair where it moves. The four-core pin does not tame it: pinned, the proxy legs read 10–20 ms against 1.6–9 ms unpinned (PERFORMANCE.md §Measuring reliably). |
...
| `listeners.https.requests` as completed handshakes | It counts SNI verdicts per connection plus requests inside intercepted sessions (API.md), so it rises on every rejected client too. Read `handshakes_completed`, or the event stream for `https` items carrying a request. Misread on 2026-09-11 and corrected the same day (code-review/phase3/p3-06-n3-alert-ab.md). |
