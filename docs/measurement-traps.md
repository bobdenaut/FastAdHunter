...
| Upstream `failures` as client timeouts | `UpstreamPool::forward` bumps a server's `failures` per *attempt* but returns `Ok` as soon as any server answers. A primary drop rescued by the fallback is a **client success**. |
