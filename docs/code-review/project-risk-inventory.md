...
| F10 | material | recommendation (undocumented gap) | `fastadhunter/src/main.rs:589`, `fah-stats/src/stats.rs:186,209` | A clean stop never flushes stats. `Engine::shutdown` aborts `spawn_snapshot_scheduler` and `spawn_history_scheduler` (wired `main.rs:534-535`); nothing calls `save_snapshot` or `flush_history` afterwards and `fah-stats` has no `Drop`. Every SIGTERM/`container stop` loses up to `snapshot_interval_seconds` (300 s default, CONFIGURATION.md:268) of aggregates, top-N and client registry. The only "no flush at shutdown" decision on record (p2.5-06) covers the failure-run length, not the snapshot. One `await` before `engine.shutdown()` returns, or a sentence in ARCHITECTURE.md saying the loss is accepted — either closes it. |
...
- None scheduled. F1, F2 and F10 are the candidates for a task when Phase 3
  closes; F11 pairs with F10 if it is taken. F3–F9, F12 are record-only.
