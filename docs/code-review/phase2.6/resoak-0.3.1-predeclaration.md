...
| 2026-09-01T11:35–11:46Z | **p5-10 Stage B collection**, evidence under [`../phase5/p5-10-stageb/`](../phase5/p5-10-stageb/). ~130 reads of `/health`, `/api/v1/telemetry`, `/api/v1/cache`, `/api/v1/debug/memory`; 5 sequential `POST /api/v1/auth/login`; one dashboard session (Dashboard 11:40:07–11:40:37Z, then Live Feed 11:40:37–11:46:20Z). **No `/history/perf` pull — the fat path was not touched.** Measured RSS impact, each against the reading immediately before it: probes 52.63 → 52.26 MiB (none); logins +0.22 MiB; dashboard +0.16 MiB; Live Feed +1.59 MiB, flat across eleven 30 s samples (54.79–55.56 MiB, no growth). `peak_rss` **unchanged at 141.34 MiB throughout**; post-session RSS 54.40 MiB. G4 clean at 11:46Z: `events_dropped` 0, `swr.dropped` 0 |
| 2026-09-01T22:20Z | ad-hoc pull — [`resoak-0.3.1/pull-adhoc-20260901T2220Z-*`](resoak-0.3.1/), 15-field set, 149 rows, `stride 1`, 16 keys, `upstreams` absent. Taken at T0+14.9 h, ahead of the pull-1 window (T0+24 h ± 1 h). No origin probe with it — the 11:15Z probe already covers the day |
| 2026-09-01T22:35–22:44Z | **the validator-armed 48 h wave** predicted by G5a's declared expectation (`~2026-09-01T22:36Z`), read from the container log at the 09-02T05:17Z pull, not observed live. 13 lists refreshed in four batches: `big.oisd.nl` 22:35:32Z; a 10-list batch 22:36:30–22:36:34Z (`filter_43` unchanged at source, the other nine refreshed); `tif-mini` 22:37:32Z; `phishdestroy` 22:44:28Z, no content change, compile skipped. Counters across the wave: `bodies` 3 → 15, `not_modified` 1 → 2, `bytes_fetched` 3 987 194 → 28 366 840 B |
| 2026-09-02T05:17Z | ad-hoc pull — [`resoak-0.3.1/pull-adhoc-20260902T0517Z-*`](resoak-0.3.1/), 15-field set, 219 rows, `stride 1`, 16 keys, `upstreams` absent. Taken at T0+21.8 h, ahead of the pull-1 window (T0+24 h ± 1 h), to capture the 22:35Z wave |
| 2026-09-02T05:19Z | origin probe, 16 conditional GETs from the dev box — 1× 304 (`dyndns`), 15× 200. Rows appended to [`resoak-0.3.0/origin-log.tsv`](resoak-0.3.0/origin-log.tsv) |
| 2026-09-02T08:15Z | **pull 1** — [`resoak-0.3.1/pull1-*`](resoak-0.3.1/), 15-field set, 248 rows, `stride 1`, 16 keys, `upstreams` absent. Taken at T0+24.8 h, inside the T0+24 h ± 1 h window (closes 08:27:49Z). First window with a G1 figure |
| 2026-09-02T22:46Z | **second `phishdestroy` recompile** (12 h list), read from the pull-2 series. `bodies` 15 → 16, `bytes_fetched` +3 735 698. Carries a `peak_rss` step of +5.81 MiB (141.34 → 148.71), attributed under G3 |
| 2026-09-03T07:41Z | **pull 2** — [`resoak-0.3.1/pull2-*`](resoak-0.3.1/), 15-field set, 483 rows, `stride 1`, 16 keys, `upstreams` absent. Taken at T0+48.2 h, in the second half of the window per the cadence adopted after pull 1 |
| 2026-09-03T07:42Z | origin probe, 16 conditional GETs from the dev box — 16× 200, every validator rotated. Rows appended to [`resoak-0.3.0/origin-log.tsv`](resoak-0.3.0/origin-log.tsv) |
