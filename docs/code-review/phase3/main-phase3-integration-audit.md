...
| F8 | **Holds.** Owner's call | `udp_max_inflight: 0` (`fah-config/src/schema/dns/mod.rs:29`), `tcp_max_connections` still defaulted from `default_tcp_max_connections()` at `:28`. **One consequence this row did not state, added 2026-09-15:** the inert bound also makes the gauge inert. `admit()` returns before touching `active` or `peak`, so `dns_udp_inflight` reports `peak: 0` on a loaded build and that zero is not evidence of anything — the 0.3.4 soak's 3.3 days reported it beside a real `dns_tcp_connections.peak` of 33. A ceiling that is off therefore cannot be sized from its own telemetry, however long the soak runs ([measurement-traps.md](../../measurement-traps.md) §A metric can be present and not measuring) |
