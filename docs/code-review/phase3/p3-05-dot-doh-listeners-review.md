...
## Findings — consolidated 2026-09-03; full history: `git show e35203e:docs/code-review/phase3/p3-05-dot-doh-listeners-review.md`

Two reviews and four fix rounds; N3 (DoT closed posture) landed in p3-06
post-review work A. Fixed and withdrawn items are omitted — git has them; the
device-side items live in §Known limitations / deferred above and are not
repeated here. Still open:

| id(s) | Issue | Status | Where |
| --- | --- | --- | --- |
| N1 | `prewarm()` runs `spawn_blocking` on every SNI hello when a CA exists, hit or miss, with two `String` allocations per handshake | deferred | p3-06 review D10 (4 µs on x86, leave alone) → P5 re-checks on the device |
| N11 | the `prewarm().await` sits between the two `timeout_at` windows, so the permit and task are held for the blocking-pool wait plus the mint on top of the deadline | deferred | p3-06 review P5 (same reading as N1) |
| N8 | with a CA installed every LAN client can drive one mint per DoT hello for any SNI it names (default-on 853); bounded by 64 permits and single-flight, unmeasured | deferred | p3-06 review §Runbook 6 watch item (d), mint rate |
| N2 | axum-built rejections (413 body limit, 400 bad `?dns=`) carry no `Cache-Control: no-store` | won't-fix | not cacheable, never `application/dns-message` |
| N4 | the DoT accept + `RetryPolicy` loop duplicates `tcp::run` except for the semaphore | won't-fix | ~20 lines; revisit at a third identical loop |
