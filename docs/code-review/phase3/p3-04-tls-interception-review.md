...
| `BASELINE_EXCLUSIONS` | the shipped 34-entry list is a first cut. The owner's banking app is **UniCredit**, already covered through `unicredit.ro`, so Runbook 4 has an excluded arm without any change to the constant. The contract is now pinned on the wire by `a_baseline_bank_is_never_intercepted_even_for_a_listed_client` | list open (extension proposed, not applied); contract tested | p3-06 review §Post-review work B, §Runbook 4 |
