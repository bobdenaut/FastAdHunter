...
covered the API surface anyway, so `p2-09` carries its own on-device check
instead. The phase is not done until both are.

**Rewritten 2026-08-06.** `p2-09` was "Query Log Reader" — make the persisted
segments searchable through `GET /api/v1/queries`. It is now API telemetry
consolidation, and it *removes* that endpoint. The reversal holds because
`[query_log] enabled = false` in the deployment (nothing is being written to the
segments) and the websocket is the live view a dashboard actually uses. The old
task file is in git history if the reader is ever wanted back.
