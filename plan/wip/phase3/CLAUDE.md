...
| 10 | `p3-10-post-merge-performance.md` | Post-merge coverage gaps (Track A — **all ten closed 2026-09-13**: six by a recorded read or owner decision, and A1/A3/A4/A7 by tests written here, each shown to fail with its own wiring broken and then reverted) and Phase 3 performance characterization (Track B). **B1 ran 2026-09-13** — `docs/code-review/phase3/p3-10-track-b1-x86.md` — and **does not close the W1 gate**: the splice rows moved 44–101% between two runs of identical code, so no percentage claim rests on them. **Those rows are no longer owed: a probe container on the RB5009 settled them on 2026-09-14** (`p3-10-track-b2-rb5009.md`) — no deploy needed, the live resolver never stopped, both runs repeated inside ±5 %, `pick: none` on every candidate, and x86 `splicebench` is not run again. Open here: the two DoT/DoH rows and splice RSS (no verified runner). The DoT/DoH rows wait on more than the missing load generator — `DoT / DoH added latency vs UDP, p50` is `TBD` until p3-11 fixes it on the device, so the order is deploy, then p3-11's budgets, then a generator designed against them. **B2 is no longer blocked as a whole** — its rustls AES-GCM row ran 2026-09-15 by probe, again with no deploy: AES-128-GCM ~1.0 ms/MiB per core, so line-rate gigabit encryption is ~12 % of one core of four, and the same arm gives an independent 9.3× check on the x86 conversion factor. What stays BLOCKED is the **N sweep**, on the deploy decision and on p3-11's dst-nat 443 rule, together with the three rows that need the live listener; the sweep gates on the shipped workload, W1. No production code changes; every new harness is its own deliverable | Fable | WAITING |
| 10b | `p3-10b-dot-connection-gauge.md` | Production-code follow-up from p3-10 A5. A DoT connection gauge separate from the TCP one, `active` / `peak` / `closed_oversize`, passed where `dot.rs:152` passes `None` today. Without it the first Phase 3 soak sets the final `dns.tcp_max_connections` default from half the traffic, and p3-10's B2 row "peak concurrent DoT connections" cannot be run at all. Shipped 2026-09-14 as `counters.dns_dot_connections` on `/api/v1/telemetry`, counted at accept so a stalled handshake is in the figure that sizes the 64-connection cap; review and fixes in `docs/code-review/phase3/p3-10b-dot-connection-gauge-review.md`. **Row 11's soak no longer waits on this one** | Fable | DONE |
...
| 11 | `p3-11-verification-sni-scope.md` | Phase 3 verification for what actually runs: DNS, plain HTTP, HTTPS filtered at the SNI with no decryption, DoT/DoH. Carries p3-06's non-interception arms — dst-nat 443 and its rollback, SNI/DoT/DoH budget rows, the security suite including "interception disabled ⇒ byte-identical splice", Private DNS, the deploy guide's HTTPS section — plus a **seven-day soak** on the deployed build. The dev-box half runs today; the device half waits on the deploy decision, and neither 10b nor 10c is something the soak waits for — both landed 2026-09-14. The DoT gauge is an instrument the week reads, and a silently dead acceptor is now an `error` line plus a `tasks_died` increment instead of a soak that looks clean and measured nothing. The week is still that gauge's first real use under household traffic, not a re-verification of it. **Its soak now has a prerequisite that did not exist when this row was written.** The 0.3.4 soak was stopped on day 5 (2026-09-16) with its question answered: the residual floor doubled, 19.0 → 38.6 MiB, and stayed elevated across every 12 h bucket while `accounted_bytes` held at 28.00 MiB — report and reducer at `ad795b0`, `docs/code-review/phase2.6/soak-0.3.4/README.md` §day 5. Running a seven-day verification soak on a build with a known unbounded floor measures the leak, not the phase. `8941770` (absent from 0.3.4) is the named fix and the deployed build must carry it | Fable | WAITING |
