...
| 11 | `p3-11-verification-sni-scope.md` | Phase 3 verification for what actually runs: DNS, plain HTTP, HTTPS filtered at the SNI with no decryption, DoT/DoH. Carries p3-06's non-interception arms — dst-nat 443 and its rollback, SNI/DoT/DoH budget rows, the security suite including "interception disabled ⇒ byte-identical splice", Private DNS, the deploy guide's HTTPS section — plus a **seven-day soak** on the deployed build. The dev-box half runs today; the device half waits on the deploy decision, and **the soak additionally waits on rows 10b and 10c** — it is the first real use of both instruments, not a re-verification of either | Fable | WAITING |
