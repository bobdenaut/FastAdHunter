...
Probe config, set once via `POST /api/v1/config`. The config keys are **boot**
keys (`restart_required: true`): set them, then one restart —
`/container/stop` + `start` on `fah-probe`, a router write the owner runs. The
listed client is not a config key since p3-07 — last row.
...
| Interception Document (`clients`) | `PUT /api/v1/interception {"clients":[<the listed client>],"exclude_domains":[]}` | live on the next accepted connection — not a boot key, not a config key; `GET /api/v1/config` omits it and a `POST /config` carrying it answers 422. A client not listed is admitted to the splice path |
