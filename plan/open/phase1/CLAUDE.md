...
|---|-----------|---------|-------|--------|
| 1 | `p1-01-rule-parsers.md` | 4 formats parsed + classified (DNS-active vs inactive) | Sonnet | WAITING |
| 2 | `p1-02-compiled-matcher.md` | Verdict lookup: allow > block, subdomains, <1ms, 1M domains ≤40MB | Opus | WAITING |
| 3 | `p1-03-list-lifecycle.md` | Download, validate, atomic swap, /data cache, refresh, user rules | Sonnet | WAITING |
| 4 | `p1-04-dns-pipeline.md` | UDP/TCP listeners, EDNS(0), verdict → blocked-response synthesis | Sonnet | WAITING |
| 5 | `p1-05-dns-cache.md` | Bounded sharded cache, TTL clamps, negative, serve-stale | Sonnet | WAITING |
| 6 | `p1-06-upstreams.md` | UDP/TCP + DoT + DoH, parallel fallback, DNSSEC pass-through | Sonnet | WAITING |
| 7 | `p1-07-stats-querylog.md` | QueryEvent channel, aggregates, ring + SSD segments, retention | Sonnet | WAITING |
| 8 | `p1-08-metrics.md` | Prometheus /metrics: QPS, latency histograms, cache ratio | Sonnet | WAITING |
| 9 | `p1-09-api.md` | Full API.md surface: key auth, TLS, endpoints, WS events (heavy) | Opus | WAITING |
| 10 | `p1-10-benches-integration.md` | criterion benches vs budgets + end-to-end integration tests | Sonnet | WAITING |
