...
### Files that arrive without a textual conflict
...
`connections.rs`. `phase3-06` never touched them, so git takes `main`'s copy
whole and raises nothing. They are additions, not deletions — a
`git diff main..phase3-06` displays them as removals purely because of diff
direction.

**Arriving without a conflict says nothing about compiling.** These files call
APIs the branch changed, and git cannot see that. Both `udp_inflight_cost.rs`
and `strategy_ab.rs` arrived clean and neither built: `DnsListenConfig` gained
three fields on the branch and `Server::serve` gained a parameter. What clears
a file in this list is the Step 4 gates, not the absence of a conflict marker.
...
| STATUS | What's done |
| --- | --- |
| DONE | `git merge main` run 2026-09-13; 16 conflicts, exactly the predicted set |
| DONE | `tcp.rs` — DoT reapplied on F1: permit-before-accept, 16 KiB bound, gauge and both F1 tests kept. `handle_connection` now takes `transport` **and** `gauge: Option<&TcpConnectionGauge>`; `dot.rs` passes `None`, so a DoT oversize close is bounded but uncounted (tests at Step 4) |
| DONE | `proxy.rs` — `pool_timer` / `pool_idle_timeout` / `pool_max_idle_per_host` and both reaper tests kept. The branch's shared `judge`/`emit` free functions kept, with main's allocation work reapplied inside them: no `authority` String, `emit` takes `Judged` by value (no clone); `intercept.rs` follows (tests at Step 4) |
| DONE | `forward_alloc.rs` — resolved to `main`'s version byte for byte; `git diff main` on it is empty |
| DONE | `server.rs` — F1/F2 wiring kept (`tcp_permits`, `tcp_gauge`, `udp_gauge`), `bind` takes `&DnsConfig`, DoT bind and `serve(pipeline, Option<DotTls>)` kept |
| DONE | `main.rs` — F10 flush, F11 `run`/`reap_dead_tasks` (both collections), `Supervised` names kept; HTTPS listener, `dot_addr` log, `HTTPS_ORIGIN_PORT` and the TLS proxy counters added. Refusals now sum main's split per cause across both proxies |
| DONE | `request.rs` — `absolute_url(scheme, request, host, port)`: main's no-authority-String form plus the branch's scheme. `domain.rs`, `lib.rs` — `connections` now comes from `fah-common` |
| DONE | `fah-model/lib.rs` union of both re-export lists; `config_store.rs` `BOOT_KEYS` is 18 keys, both sides' entries kept |
| DONE | `server_integration.rs` — main's `build_pipeline`/`start_server_on` split kept, every `bind` takes a `DnsConfig` (new `config_with` helper), `serve(pipeline, None)`; F1/F2 ceiling test and the DoT test both present |
| DONE | the five docs — API (main's `refused_claim`/`refused_destination` split kept, the branch's `listeners` block folded onto it), ARCHITECTURE (F1/F2 bounds plus the DoT/DoH listener rows), README, ROADMAP (Phase 3 is 🚧 again, not parked), project-state (rewritten at Step 5) |
| DONE | `Cargo.lock` — the silent three-way merge left `fah-certs` at `0.3.3` while the workspace is `0.3.4`; corrected, then `cargo check --workspace --all-targets --all-features --locked` passes clean |
| DONE | no conflict markers (`git grep` on both forms), `git diff --check` clean |
...
**It is a reporting harness, not a net.** It contains no `assert!` and no
threshold: it prints `heap_per_inflight_bytes`, `rss_growth_mib` and
`allocs_per_query` and exits green whatever they are. A green run therefore
proves only that it ran. Read the printed figures against
docs/measurement-traps.md; an F2 regression shows up as a number here, never
as a failure.
